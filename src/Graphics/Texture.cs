...
namespace GoblinXNA.Graphics
{
    ...
    /// possible errors that can happen while loading. This code was ported from
    /// XNA Racing Game project developed by eXDreams.
    /// </summary>
    public class Texture : IDisposable
    {
        ...
        {
            if (alphaSprite == null)
                alphaSprite = new SpriteBatch(State.Device);

            if (additiveSprite == null)
                additiveSprite = new SpriteBatch(State.Device);
            ...
            string fullFilename =
                Path.Combine(State.GetSettingVariable("TextureDirectory"), texFilename);

            // Try loading as 2d texture
            internalXnaTexture = State.Content.Load<Texture2D>(@"" + fullFilename);
        ...
        public void RenderOnScreenRelative16To9(int relX, int relY,
            Rectangle pixelRect)
        {
            alphaSprite.Draw(internalXnaTexture, RectangleHelper.CalcRectangle(
        ...
        public void RenderOnScreenRelative4To3(int relX, int relY,
            Rectangle pixelRect)
        {
            alphaSprite.Draw(internalXnaTexture, RectangleHelper.CalcRectangleKeep4To3(
                relX, relY, pixelRect.Width, pixelRect.Height),
                pixelRect, Color.White);
        }
        ...
        {
            alphaSprite.Draw(internalXnaTexture, RectangleHelper.CalcRectangle1600(
                relX, relY, pixelRect.Width, pixelRect.Height),
                pixelRect, Color.White);
        }
