...
namespace GoblinXNA.Graphics.ParticleEffects2D
{
    ...
    public class Particle
    {
            ...
            float lifetime, float scale, float rotationSpeed)
        {
            ...
            this.Rotation = RandomHelper.GetRandomFloat(0, MathHelper.TwoPi);
        }

        public void Initialize3D(Vector3 position, Vector3 velocity, Vector3 acceleration,
            float lifetime, float scale, float rotationSpeed)
        {
            this.Position3D = position;
            this.Velocity3D = velocity;
            this.Acceleration3D = acceleration;
            this.Lifetime = lifetime;
            this.Scale = scale;
            this.RotationSpeed = rotationSpeed;
        ...
        public void Update3D(float dt)
        {
            Velocity3D += Acceleration3D * dt;
