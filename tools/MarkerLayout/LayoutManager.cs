...
namespace MarkerLayout
{
    public class LayoutManager : IDisposable
    {
        ...
        public enum ConfigType { ARTag, ALVAR };
        ...
        public void AddBackground(String imageFilename, Point position, Size size)
        {
        ...
        /// <exception cref="InvalidOperationException"></exception>
        public void OutputConfig(String configFilename, ConfigType type, params String[] appendConfigs)
        {
            if (!coodframeConfig.CoodframeName.Equals(""))
            ...
            try
            {
                if (type == ConfigType.ARTag)
                {
                    tw.WriteLine("// This file is generated by MarkerLayout program created by " +
                        "Ohan Oda (ohan@cs.columbia.edu) \n\n");

                    foreach (KeyValuePair<CoordframeConfig, List<int>> coordframe in coordframes)
                    {
                        tw.WriteLine("coordframe");
                        tw.WriteLine("name=\"" + coordframe.Key.CoodframeName + "\"");
                        if (coordframe.Key.MinPoint > 0)
                            tw.WriteLine("min_points=" + coordframe.Key.MinPoint);
                        tw.WriteLine();

                        foreach (int markerID in coordframe.Value)
                        {
                            tw.WriteLine("\tmarker");
                            tw.WriteLine("\tid=" + markerID);
                            tw.WriteLine("\t//-list of corners cw from top left");
                            ...
                            tw.WriteLine("\t\tvtx=" + configPos.Y + "," + configPos.X + ",0\t//upper left");
                            tw.WriteLine("\t\tvtx=" + configPos.Y + "," + (configPos.X + configSize.Width)
                                + ",0\t//upper right");
                            tw.WriteLine("\t\tvtx=" + (configPos.Y + configSize.Height) + "," +
                                (configPos.X + configSize.Width) + ",0\t//lower right");
                            tw.WriteLine("\t\tvtx=" + (configPos.Y + configSize.Height) + "," + configPos.X
                                + ",0\t//lower left");
                ...
                else if (type == ConfigType.ALVAR)
                {
                    foreach (KeyValuePair<CoordframeConfig, List<int>> coordframe in coordframes)
                    {
                        ...
                        foreach (int markerID in coordframe.Value)
                            tw.WriteLine("" + configs[markerID].Size.Width * ratio);
                        tw.WriteLine();
                ...
                for (int i = 0; i < appendConfigs.Length; i++)
                {
                    FileStream appFs = new FileStream(appendConfigs[i], FileMode.Open);
                    StreamReader reader = new StreamReader(appFs);
                ...
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        else if (reader.Name.Equals("OutputConfig"))
                        {
                            ...
                            ConfigType type = ConfigType.ARTag;
                            if (reader.GetAttribute("type") != null)
                            {
                                if (reader.GetAttribute("type").Equals("ARTag"))
                                    type = ConfigType.ARTag;
                                else if (reader.GetAttribute("type").Equals("ALVAR"))
                                    type = ConfigType.ALVAR;
                            }

                            if (reader.GetAttribute("appends") != null)
                            {
                                String[] appends = reader.GetAttribute("appends").Split(',');
                                OutputConfig(name, type, appends);
                            }
                            else
